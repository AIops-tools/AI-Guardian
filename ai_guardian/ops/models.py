"""Model inventory, runtime state, and lifecycle over the Ollama API.

Reads annotate each installed/running model with the allow/deny policy verdict so
shadow (unsanctioned) models are visible at a glance. Writes cover pull / remove
/ unload; ``remove_model`` captures the model's manifest (name+digest+license)
before deleting so the harness can record an undo (re-pull) descriptor.
"""
