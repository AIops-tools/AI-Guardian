"""``ai-guardian secret`` — manage the encrypted credential store.

Secrets (AI Guardian API keys) are stored in ``~/.ai-guardian/secrets.enc``
...
from ai_guardian.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from ai_guardian.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: ai-guardian secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update AI_GUARDIAN_AIOPS_MASTER_PASSWORD.[/]")
