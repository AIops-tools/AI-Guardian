"""Encrypted secret storage for ai-guardian.

Secrets (AI Guardian API keys) are NEVER written to disk in plaintext. They
live in ``~/.ai-guardian/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``AI_GUARDIAN_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
MASTER_PASSWORD_ENV = "AI_GUARDIAN_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``AI_GUARDIAN_<TARGET>_PASSWORD``; ``prefix='AI_GUARDIAN_'`` and
