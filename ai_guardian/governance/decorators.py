"""The ``@governed_tool`` decorator — mandatory wrapper for all AI Guardian MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.ai-guardian/audit.db``
...
from ai_guardian.governance.audit import detect_agent, get_engine
from ai_guardian.governance.budget import BudgetExceeded, get_budget
from ai_guardian.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from ai_guardian.governance.patterns import PatternMatch, get_pattern_engine
from ai_guardian.governance.policy import PolicyResult, get_policy_engine
from ai_guardian.governance.readonly import is_read_only, read_only_denial
from ai_guardian.governance.sanitize import sanitize

_log = logging.getLogger("ai-guardian.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all AI Guardian MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.ai-guardian/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("AI_GUARDIAN_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("AI_GUARDIAN_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (AI_GUARDIAN_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set AI_GUARDIAN_AUDIT_APPROVED_BY to the authorizing human (and "
            f"AI_GUARDIAN_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`ai_guardian.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from ai_guardian.governance.undo import get_undo_store
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight AI Guardian calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``ai_guardian.ops.jobs`` → ``ai-guardian``
    ``mcp_server.server`` → ``ai-guardian`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("ai_guardian") or module.startswith("mcp_server"):
        return "ai-guardian"
    return "unknown"
