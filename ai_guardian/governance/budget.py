...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``AI_GUARDIAN_RUNAWAY_MAX`` times within ``AI_GUARDIAN_RUNAWAY_WINDOW_SEC`` trips a
...
from ai_guardian.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing AI Guardian-specific; the decorator wraps it into a
