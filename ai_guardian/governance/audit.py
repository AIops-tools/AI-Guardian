"""Unified audit logging engine — all AI Guardian skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.ai-guardian/audit.db``.
...
_log = logging.getLogger("ai-guardian.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring AI_GUARDIAN_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the AI_GUARDIAN_AIOPS_HOME refactor
