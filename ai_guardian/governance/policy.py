"""Policy engine — rule-based access control for AI Guardian MCP tools.

Rules are loaded from ``~/.ai-guardian/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("ai-guardian.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("AI_GUARDIAN_POLICY_DISABLED") == "1":
        ...
        if not tiers:
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        f"No rules file at {self._path} — high/critical operations require a named approver (AI_GUARDIAN_AUDIT_APPROVED_BY) by default. Author rules.yaml "
