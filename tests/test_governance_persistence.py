...
throwaway home (``AI_GUARDIAN_AIOPS_HOME``) and assert that the rows compliance
...
from ai_guardian.governance import PolicyDenied, governed_tool
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("AI_GUARDIAN_AUDIT_APPROVED_BY", raising=False)
    monkeypatch.delenv("AI_GUARDIAN_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="local")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="local")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("AI_GUARDIAN_AUDIT_APPROVED_BY", "ops-alice")
    result = _drop_widget(name="w3", target="local")
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="local")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch, fake_ollama):
    """End-to-end through a REAL product write tool: remove_model must capture
    the model's manifest before deleting and persist the re-pull undo on disk."""
    monkeypatch.setenv("AI_GUARDIAN_AUDIT_APPROVED_BY", "ops-alice")
    manifest = {"license": "MIT License", "details": {"family": "llama"}, "capabilities": []}
    ...
    result = gov.remove_model(model="llama3.2:3b")
    assert result["priorState"]["model"] == "llama3.2:3b"
    assert result["priorState"]["license"] == "MIT License"
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "pull_model"
    assert "llama3.2:3b" in undo[0]["undo_params"]

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["remove_model"]
