...
``ai_guardian.governance.audit`` and binds state to an isolated home via
...
from ai_guardian.governance.audit import (
    AuditEngine,
    _current_user,
    _safe_json,
    detect_agent,
...
def test_query_filters(audit_home):
    eng = AuditEngine(audit_home / "audit.db")
    eng.log(skill="ai-guardian", tool="a", status="ok", workflow_id="w1")
    eng.log(skill="ai-guardian", tool="b", status="error", workflow_id="w2")
    eng.log(skill="other", tool="a", status="ok", workflow_id="w1")

    assert len(eng.query()) == 3
    assert len(eng.query(skill="ai-guardian")) == 2
...
def test_stats_aggregation(audit_home):
    eng = AuditEngine(audit_home / "audit.db")
    eng.log(skill="ai-guardian", tool="a", status="ok")
    eng.log(skill="ai-guardian", tool="b", status="error")
    ...
    assert stats["by_skill"] == {"ai-guardian": 2, "other": 1}
