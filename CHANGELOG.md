...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/ai-guardian` to
  `@zw008/ai-guardian`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/ai-guardian`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/ai-guardian`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install ai-guardian@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/ai-guardian`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [ai-guardian, uvx]` — either one suffices — with every variable kept
...
### Fixed
- **A mistyped block threshold silently disabled risk blocking — in the shipped opt-in path as well as the new proxy.** `_band_ge` looked both names up in an ordered tuple and swallowed the `ValueError`, answering "not at or above the threshold". So `--block-threshold HIGH`, differing from `high` only in case, made **every** prompt pass: a critical AWS-key finding sailed through while the CLI's startup banner still announced the threshold. The same helper is used by `guarded_generate` and `observe_chat`, so a caller passing "HIGH" disabled their gate too. There is now one definition in `scanner` (`BANDS`, `normalize_band`, `band_at_or_above`): comparison is case-insensitive, an unknown **threshold** raises — at proxy construction, so it fails to start rather than serving traffic while enforcing nothing — and an unknown **band** fails *closed*, because an unclassifiable risk should block rather than pass.
