# Release notes — ai-guardian 0.5.0

Previous release: 0.4.0.

## In this tool

- **`remove_model` no longer records an undo it knows will be refused.** Its inverse is `pull_model`, which the tool's own policy engine rejects for a denied model — so the natural sequence (deny a model, then remove it) left a token that looked valid in `undo_list` and failed at `undo_apply`. Now the result says the removal is not reversible and no token is written.
